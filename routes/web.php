...
use App\Http\Controllers\SchemeProductController;
use App\Http\Controllers\ResearchAreasController;
use App\Http\Controllers\MeteorologyStationsController;
use App\Http\Controllers\StationsController;
use App\Http\Controllers\LandslideLocationsController;
...
Route::group(['middleware' => 'guest'], function () {
    ...
    Route::post('dang-nhap', [LoginController::class, 'post'])->name('post-login');
});
...
Route::group([
    ...
    // Muc tieu de tai
    Route::get('muc-tieu-chung', [GeneralInfoController::class, 'commonTarget'])->name('thong-tin-chung.muc-tieu-chung');
    Route::get('muc-tieu-cu-the', [GeneralInfoController::class, 'specificTarget'])->name('thong-tin-chung.muc-tieu-cu-the');

    // San pham de tai
    Route::get('san-pham-de-tai', [SchemeProductController::class, 'showAll' ])->name('thong-tin-chung.san-pham-de-tai');
    ...
    Route::get('san-pham-de-tai/{Scheme_ID}', [SchemeProductController::class, 'getDetail'])->name('thong-tin-chung.chi-tiet-san-pham-de-tai');
    // Sua ten san pham de tai
    Route::get('san-pham-de-tai/{Scheme_ID}/edit', [SchemeProductController::class, 'showEdit'])->name('thong-tin-chung.sua-san-pham-de-tai');
    Route::post('san-pham-de-tai/{Scheme_ID}/edit', [SchemeProductController::class, 'doEdit']);
...
Route::group([
    ...
    // Hien thi
    Route::get('{slug}', [ResearchAreasController::class, 'index']);

    // Chinh sua
    Route::get('{slug}/edit', [ResearchAreasController::class, 'showEdit']);
    Route::post('{slug}/edit', [ResearchAreasController::class, 'doEdit']);
});
...
Route::group([
    ...
    //So lieu khi tuong
    Route::get('so-lieu-khi-tuong', [MeteorologyStationsController::class, 'showStations'])->name('du-lieu-de-tai.so-lieu-khi-tuong');
    Route::get('so-lieu-khi-tuong/{id}',[MeteorologyStationsController::class, 'getRainData']);

    //So lieu bun cat
    Route::get('so-lieu-bun-cat', [StationsController::class, 'muddySandStations'])->name('du-lieu-de-tai.so-lieu-bun-cat');
    Route::get('so-lieu-bun-cat/{id}',[StationsController::class, 'getMuddySandData']);

    // So lieu thuy van
    Route::get('so-lieu-thuy-van', [StationsController::class, 'hydrologicalStations'])->name('du-lieu-de-tai.so-lieu-thuy-van');
    Route::get('so-lieu-thuy-van/{id}',[StationsController::class, 'getHydrologicalData']);
...
Route::group([
    ...
    Route::get('2020', [LandslideLocationsController::class, 'year2020'])->name('hien-trang-sat-lo-2020');
...
// Ban do nguy co sat lo
Route::get('import-file', [ImportController::class, 'showImport'])->name('import-file');
Route::post('import-file', [ImportController::class, 'doImport'])->name('do-import-file');